...
apiClient.interceptors.response.use(
    (response) => {
      // 如果是 blob 类型（文件下载），返回完整的 response 对象（包含 data 和 headers）
...
export const orderService = {
   ...
   * @param {Number|String} ticketId - 票ID
   * @returns {Promise} 返回包含 blob 数据和响应头的对象 { data: Blob, headers: Object }
   */
  downloadTicket(ticketId) {
    return apiClient.get(`/tickets/download/${ticketId}`, {
